...
if __name__ == "__main__":
    ...
    audio_file = "../string_sample/heymemo/all_heymemo.wav"
    ...
    model_quant_thresh = 0.999 #分为0.9~0.999
    ...
    flow_quant_thresh =0.999 #范围0.9~0.999
    ...
    for i in range(round(frame_cnt)):
        ...
        out_np = fc_out.detach().cpu().numpy() if isinstance(fc_out, torch.Tensor) else np.array(fc_out)
        x = out_np.reshape(-1)
        e_x = np.exp(x - np.max(x))
        sm = e_x / e_x.sum()
        logits_str = " ".join(str(v) for v in x.tolist())
        softmax_str = " ".join(str(p) for p in sm.tolist())
        # 计算并写入时间戳（从 0.00s 开始，每步 +0.02s）
        timestamp = i * frame_num / 16000
        logf.write(f"time:{timestamp:.2f}s, [[ {logits_str}  ]], softmax: [ {softmax_str} ]\n")
        logf.flush()
